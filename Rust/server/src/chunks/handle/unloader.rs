...
pub async fn init(chunk_handle: Handle, player_handle: player::handle::Handle) {
    let forever = task::spawn(async move {
        ...
        loop {
            ...
            unload(p_coords, c_coords, chunk_handle.clone()).await;
        }
    });
...
async fn unload(player_coords: Vec<Coord>, chunk_coords: Vec<Coord>, chunk_handle: Handle) {
    ...
    for chunk in chunk_coords.iter() {
        ...
        for player in player_coords.iter() {
            ...
            let distance = (((player[0] - chunk[0]).pow(2)
                + (player[1] - chunk[1]).pow(2)
                + (player[2] - chunk[2]).pow(2)) as f64)
                .sqrt() as i64;
