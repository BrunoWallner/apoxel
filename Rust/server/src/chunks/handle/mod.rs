...
pub enum Instruction {
    ...
    RegisterUpdateNotifier {
        token: Token,
        sender: mpsc::Sender<ClientChunkEvent>,
    },
}
...
impl Handle {
    ...
    pub async fn request(&self, chunks: Vec<Coord>, token: Token) {
        self.sender
            .send(Instruction::RequestChunks { chunks, token })
            .await
            .unwrap();
    }
    pub async fn get_keys(&self) -> Vec<Coord> {
        ...
        self.sender
            .send(Instruction::RequestKeys(tx))
            .await
            .unwrap();
    ...
    pub async fn place_structure(&self, coord: Coord, structure: Structure) {
        self.sender
            .send(Instruction::PlaceStructure { coord, structure })
            .await
            .unwrap();
    }
    pub async fn push_chunks(&self, chunk: SuperChunk) {
        self.sender
            .send(Instruction::PushChunks(chunk))
            .await
            .unwrap();
    }
    pub async fn register_update_notifier(
        &self,
        token: Token,
        sender: mpsc::Sender<ClientChunkEvent>,
    ) {
        self.sender
            .send(Instruction::RegisterUpdateNotifier { token, sender })
            .await
            .unwrap();
    }
}
...
async fn init(
    mut receiver: mpsc::Receiver<Instruction>,
    handle: Handle,
                            ...
                            if let Some(coord) = coords.pop() {
                                ...
                                if let Some(chunk) = chunks.get_mut(&coord) {
                                    if let Some(sender) = update_register.get(token) {
                                        sender
                                            .send(ClientChunkEvent::PushLoadedChunk(*chunk))
                                            .await
                                            .unwrap();
                                    }
                                } else {
